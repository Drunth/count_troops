ScriptAPI.register('100-UnterstÃ¼tzung zÃ¤hlen', true, 'Ademes', 'support-nur-im-forum@ademes.at')
...
if (!doc.URL.match('mode=units')) {
  UI.InfoMessage('Du musst dich auf der Verteidigungs-/UnterstÃ¼tzungs-Ãœbersicht befinden!', 3000, true)
} else {
...
function ADS_Unterstuetzung_zaehlen (doc) {
  ...
  if (doc.URL.match('away_detail')) {
    ...
    output += '<h5>UnterstÃ¼tzungen erhalten von:</h5>'
    word = 'Erhalten'
  }
  var counter = 0
  var sum = 0
  ...
  output += '<th style="padding:4px;">Account</th><th style="padding:4px;">Speer</th><th style="padding:4px;">Schwert</th><th style="padding:4px;">Skav</th style="padding:4px;"><th style="padding:4px;">Spy</th>'
